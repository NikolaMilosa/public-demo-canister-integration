...
// Proxy that lets callers use `backend.method()` directly — each call
// waits for initialization, then forwards to the real backend.
export const backend = new Proxy({}, {
  get(_target, prop) {
    return (...args) => backendReady.then((b) => b[prop](...args));
  },
});
